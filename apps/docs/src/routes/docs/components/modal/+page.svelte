...
<div data-docs-page class="flex flex-col gap-10">
	...
	<header class="flex flex-col gap-4">
		<div>
			...
			>
				The low-level overlay primitive used by Dialog, AlertDialog, and custom flows. You probably
				want one of those — reach for Modal directly only when you need a custom surface shape.
			</p>
		</div>
	</header>
	...
	<section id="api" class="scroll-mt-20 flex flex-col gap-4">
		...
		<p class="text-sm text-foreground-muted">
			Compose Modal with Root, Trigger, Content, and layout sub-components.
		</p>
				...
				{#each apiRows as row, i (i)}
					<li class="grid grid-cols-[1fr_1.8fr_0.5fr] gap-3 px-4 py-3 max-md:grid-cols-1">
						<div class="flex items-center gap-2">
							<button
								type="button"
								onclick={() => copy(`Modal.${row.component}`, `prop-${row.component}`)}
