<script lang="ts">
	...
	const heroCode = `<Select.Root value={role}>
  <Select.Trigger>Select a role</Select.Trigger>
  <Select.Content>
    <Select.Item value="designer" onclick={() => (role = 'designer')}>Designer</Select.Item>
    <Select.Item value="engineer" onclick={() => (role = 'engineer')}>Engineer</Select.Item>
  </Select.Content>
</Select.Root>`;
...
<div data-docs-page class="flex flex-col gap-10">
	...
	<header class="flex flex-col gap-4">
		<div>
			...
			>
				A controlled dropdown for short option lists. Once the list goes past approximately 10
				items, switch to Combobox so users can type to filter.
			</p>
		</div>
	</header>
	...
	<section id="hero" class="scroll-mt-20 flex flex-col gap-4">
		<ComponentPreview code={heroCode}>
			<Select.Root value={pgRole}>
				<Select.Trigger class="h-9 w-60" variant="outline">
	...
	<section id="installation" class="scroll-mt-20 flex flex-col gap-4">
		...
		<p class="text-sm text-foreground-muted">Install the Select component with the CLI:</p>
		<Steps
			steps={[
				{
					title: 'Run the CLI',
	...
	<section id="api" class="scroll-mt-20 flex flex-col gap-4">
		...
		<p class="text-sm text-foreground-muted">
			Compose Select with Root, Trigger, Content, and Item sub-components.
		</p>
					...
					<li class="grid grid-cols-[1fr_1.8fr_0.5fr] gap-3 px-4 py-3 max-md:grid-cols-1">
						<div class="flex items-center gap-2">
							<button
								type="button"
								onclick={() =>
									copy(`Select.${row.component}.${row.prop}`, `prop-${row.component}-${row.prop}`)}
							...
							>
								...
								>
									Select.{row.component} <span class="text-foreground-muted">{row.prop}</span>
								</code>
								{#if copiedSnippet === `prop-${row.component}-${row.prop}`}
