...
<div data-docs-page class="flex flex-col gap-10">
	...
	<section id="api" class="scroll-mt-20 flex flex-col gap-4">
		...
		<p class="text-sm text-foreground-muted">
			Accordion exports <code class="font-mono text-foreground">Root</code>,
					...
					<li class="grid grid-cols-[1fr_1.8fr_0.5fr] gap-3 px-4 py-3 max-md:grid-cols-1">
						<div class="flex items-center gap-2">
							<button
								type="button"
								onclick={() =>
									copy(`${row.component}.${row.prop}`, `prop-${row.component}-${row.prop}`)}
	...
	>
		<div class="flex flex-col gap-1">
			...
			<p class="m-0 text-[0.86rem] text-foreground-muted">
				Every Accordion token lives in the studio — restyle it for your brand in seconds.
			</p>
		</div>
