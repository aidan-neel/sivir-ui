<script lang="ts">
	...
	const apiRows = [
		{
			...
			type: 'string',
			default: '--',
			description: 'Required. The string written to the clipboard on click.'
		},
		{
			prop: 'label',
			type: 'string',
			default: '"Copy"',
			description: 'Tooltip label at rest.'
		},
		{
			prop: 'copiedLabel',
			type: 'string',
			default: '"Copied"',
			description: 'Tooltip label shown right after copying (rolls in via slot-text).'
		},
		{
			prop: 'duration',
			type: 'number',
			default: '2000',
			description: 'How long (ms) to hold the copied state before reverting.'
		},
		{
			prop: 'variant',
			type: 'ButtonVariant',
		...
		},
		{
			...
			type: '"sm" | "md" | "lg" | "icon"',
			default: '"icon"',
			description: 'Underlying Button size.'
		},
		{
			prop: 'oncopy',
			type: '(text: string) => void',
			default: '--',
			description: 'Fired after the text is successfully written to the clipboard.'
		}
	];
	...
	const installCommand = 'bunx @aidan-neel/ui add copy-button';
</script>

<svelte:head>
	<title>Silk · Copy Button</title>
	<meta
		name="description"
...
<div data-docs-page class="flex flex-col gap-10">
	...
	<header class="flex flex-col gap-4">
		<div>
			...
			>
				Copies text to the clipboard in one tap. The icon morphs from copy to check for a beat, and
				the tooltip label rolls between states with <code class="font-mono">slot-text</code>.
			</p>
		</div>
	</header>
	...
	<section id="api" class="scroll-mt-20 flex flex-col gap-4">
		...
		<p class="text-sm text-foreground-muted">
			<code class="font-mono text-foreground">CopyButton</code> takes the text to copy plus a few presentation
			props.
		</p>
