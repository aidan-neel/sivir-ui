<script lang="ts">
	...
	import InsetSurface from './examples/inset-surface.svelte';
	import InsetSurfaceSrc from './examples/inset-surface.svelte?raw';
	...
	const apiRows = [
		{
			...
			default: '—',
			description:
				'Extra classes, merged with the defaults via tailwind-merge (yours win on conflicts). Override bg, radius, or padding here.'
		},
		{
			prop: 'children',
			type: 'Snippet',
			default: '—',
			description: 'Content rendered inside the frame.'
		}
	];
...
<svelte:head>
	...
	<meta
		name="description"
		content="A surface container framed by a concentric double-line border."
	/>
</svelte:head>
...
<div data-docs-page class="flex flex-col gap-10">
	...
	<header class="flex flex-col gap-4">
		<div>
			...
			>
				A plain surface with one trick: a concentric double-line border. The inner hairline is inset
				and radius-matched, so both corners stay parallel. Wrap anything in it.
			</p>
		</div>
	</header>
	...
	<section id="usage" class="scroll-mt-20 flex flex-col gap-4">
		...
		<p class="text-sm text-foreground-muted">Import Panel and wrap your content:</p>
		<CodeBlock
			code={`import { Panel } from '@silk/ui/components/panel';\n\n<Panel>\n\tYour content\n</Panel>`}
			lang="svelte"
			copy="overlay"
		/>
	</section>
	...
	<section id="examples" class="scroll-mt-20 flex flex-col gap-10">
		<div>
			...
			<p class="mt-2 text-sm text-foreground-muted">
				The frame on its own, and as a layered inset.
			</p>
		</div>

		<!-- Inset surface -->
		<div id="inset-surface" class="scroll-mt-20 flex flex-col gap-3">
			<h3
			...
			<p class="text-sm text-foreground-muted">
				Drop the frame a hair darker than a card-coloured child and the content reads as a distinct
				inset — the pattern the component preview on this site uses.
			</p>
			<ComponentPreview code={InsetSurfaceSrc}>
				<InsetSurface />
			</ComponentPreview>
		</div>
	</section>
	...
	<section id="api" class="scroll-mt-20 flex flex-col gap-4">
		...
		<p class="text-sm text-foreground-muted">
			<code class="font-mono text-foreground">Panel</code> is the only export. The border reads from
			your theme tokens — tune <code class="font-mono text-foreground">--panel-padding</code>,
			<code class="font-mono text-foreground">--panel-inset</code>,
			<code class="font-mono text-foreground">--radius-lg</code>, and
			<code class="font-mono text-foreground">--border-size</code>.
		</p>
