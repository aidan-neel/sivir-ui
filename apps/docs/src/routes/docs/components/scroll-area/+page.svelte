<script lang="ts">
	...
	const SOURCE = 'https://github.com/aidan-neel/silk/tree/main/registry/silk/default/scroll-area';
	...
	const installCommand = 'bunx @aidan-neel/ui add scroll-area';
	const heroCode = `<ScrollArea class="h-48 w-64 rounded-md border">
  <div class="flex flex-col">
    {#each items as item}
      <div class="border-b px-3 py-2">{item}</div>
    {/each}
  </div>
</ScrollArea>`;
</script>
...
<div data-docs-page class="flex flex-col gap-10">
	...
	<section id="installation" class="scroll-mt-20 flex flex-col gap-4">
		...
		<p class="text-sm text-foreground-muted">Install the Scroll Area component with the CLI:</p>
		<Steps
			steps={[
				{
					title: 'Run the CLI',
	...
	<section id="usage" class="scroll-mt-20 flex flex-col gap-4">
		...
		<p class="text-sm text-foreground-muted">Import the Scroll Area and use it to wrap content:</p>
		<pre
	...
	<section id="api" class="scroll-mt-20 flex flex-col gap-4">
		...
		<p class="text-sm text-foreground-muted">
			<code class="font-mono text-foreground">ScrollArea</code> accepts a single prop to control scrolling
			behavior.
		</p>
