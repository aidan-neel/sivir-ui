<script lang="ts">
	...
	const heroCode = `<Calendar bind:value={date} />`;
</script>

<svelte:head>
	<title>Silk · {TITLE}</title>
	<meta name="description" content="A single-date calendar with locale-aware formatting." />
</svelte:head>
...
<div data-docs-page class="flex flex-col gap-10">
	...
	<section id="installation" class="scroll-mt-20 flex flex-col gap-4">
		...
		<p class="text-sm text-foreground-muted">Install the Calendar component with the CLI:</p>
		<Steps
			steps={[
				{
					title: 'Run the CLI',
	...
	<section id="usage" class="scroll-mt-20 flex flex-col gap-4">
		...
		<p class="text-sm text-foreground-muted">Import and use the Calendar component:</p>
		<pre
	...
	<section id="examples" class="scroll-mt-20 flex flex-col gap-10">
		...
		<!-- With constraints -->
		<div id="with-constraints" class="scroll-mt-20 flex flex-col gap-3">
			<h3
				class="text-[1rem] font-[var(--font-weight-header,600)] tracking-tight text-foreground docs-subsection-heading"
			>
			...
			>
				<div class="flex items-center justify-center">
					<Calendar bind:value={date} min={new Date(2024, 0, 1)} max={new Date(2024, 11, 31)} />
				</div>
			</ComponentPreview>
		</div>
	</section>
	...
	<section id="api" class="scroll-mt-20 flex flex-col gap-4">
		...
		<p class="text-sm text-foreground-muted">
			<code class="font-mono text-foreground">Calendar</code> is the only export.
		</p>
	...
	>
		<div class="flex flex-col gap-1">
			...
			<p class="m-0 text-[0.86rem] text-foreground-muted">
				Every Calendar token lives in the studio — restyle it for your brand in seconds.
			</p>
		</div>
