<script lang="ts">
	...
	const variantList: { value: Variant; label: string }[] = [
	...
	const heroCode = `<Alert.Root variant="info">
...
<div data-docs-page class="flex flex-col gap-10">
	...
	<section id="installation" class="scroll-mt-20 flex flex-col gap-4">
		...
		<p class="text-sm text-foreground-muted">Install the Alert component with the CLI:</p>
		<Steps
			steps={[
				{
					title: 'Run the CLI',
	...
	<section id="examples" class="scroll-mt-20 flex flex-col gap-10">
		...
		{#each variantList as v (v.value)}
			{@const code = `<Alert.Root${v.value !== 'info' ? ` variant="${v.value}"` : ''}>\n  <Alert.Title>${v.label}</Alert.Title>\n  <Alert.Description>Alert description</Alert.Description>\n</Alert.Root>`}
				...
				<ComponentPreview {code}>
					<div class="w-full max-w-md">
						<Alert.Root variant={v.value}>
							<Alert.Title>{v.label}</Alert.Title>
							<Alert.Description>
								This is a {v.label.toLowerCase()} alert with a title and description.
							</Alert.Description>
						</Alert.Root>
					</div>
				</ComponentPreview>
	...
	<section id="api" class="scroll-mt-20 flex flex-col gap-4">
		...
		<p class="text-sm text-foreground-muted">
			<code class="font-mono text-foreground">Alert</code> exports
			<code class="font-mono text-foreground">Root</code>,
			<code class="font-mono text-foreground">Title</code>, and
			<code class="font-mono text-foreground">Description</code>.
		</p>
	...
	>
		<div class="flex flex-col gap-1">
			...
			<p class="m-0 text-[0.86rem] text-foreground-muted">
				Every Alert token lives in the studio — restyle it for your brand in seconds.
			</p>
		</div>
