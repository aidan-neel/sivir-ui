<script lang="ts">
	...
	const SOURCE = 'https://github.com/aidan-neel/silk/tree/main/registry/silk/default/radio-group';
...
<div data-docs-page class="flex flex-col gap-10">
	...
	<section id="examples" class="scroll-mt-20 flex flex-col gap-10">
		<div>
			...
			<p class="mt-2 text-sm text-foreground-muted">RadioGroup in different configurations.</p>
		</div>
	...
	<section id="api" class="scroll-mt-20 flex flex-col gap-4">
		...
		<p class="text-sm text-foreground-muted">
			Compose RadioGroup with Root and Item sub-components.
		</p>
				...
				{#each apiRows as row, i (i)}
					<li class="grid grid-cols-[1fr_1.8fr_0.5fr] gap-3 px-4 py-3 max-md:grid-cols-1">
						<div class="flex items-center gap-2">
							<button
								type="button"
								onclick={() => copy(`RadioGroup.${row.component}`, `prop-${row.component}`)}
