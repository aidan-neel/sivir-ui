<script lang="ts">
	...
	const TITLE = 'Marquee';
	const SOURCE = 'https://github.com/aidan-neel/silk/tree/main/registry/silk/default/marquee';
	...
	const heroCode = `<Marquee pauseOnHover duration="28s">\n  {#each logos as logo (logo)}\n    <span>{logo}</span>\n  {/each}\n</Marquee>`;
	const installCommand = 'bunx @aidan-neel/ui add marquee';
...
<svelte:head>
	<title>Silk · Marquee</title>
	<meta
		name="description"
		content="A CSS-only infinite scroll. Pure transforms, no JavaScript timeline — pauses on hover when you want it to."
	/>
</svelte:head>
...
<div data-docs-page class="flex flex-col gap-10">
	...
	<header class="flex flex-col gap-4">
		<div>
			...
			>
				A CSS-only infinite scroll. Pure transforms, no JavaScript timeline — pauses on hover when
				you want it to.
			</p>
		</div>
	</header>
				...
				>
					...
					>
						<Marquee pauseOnHover duration="28s" class="py-8">
							{#each logos as logo (logo)}
								<span
									class="rounded-[var(--radius-md)] border border-border bg-card px-4 py-2 text-[0.86rem] font-[var(--font-weight-label,500)] text-foreground"
									>{logo}</span
								>
							{/each}
						</Marquee>
	...
	<section id="installation" class="scroll-mt-20 flex flex-col gap-4">
		...
		<p class="text-sm text-foreground-muted">Install the Marquee component with the CLI:</p>
		<Steps
			steps={[
				{
					title: 'Run the CLI',
	...
	<section id="usage" class="scroll-mt-20 flex flex-col gap-4">
		...
		<p class="text-sm text-foreground-muted">Import the Marquee and wrap your content:</p>
		<pre
	...
	<section id="examples" class="scroll-mt-20 flex flex-col gap-10">
		<div>
			...
			<p class="mt-2 text-sm text-foreground-muted">
				Marquee in different configurations and use cases.
			</p>
		</div>

		<!-- Pause on hover -->
		<div id="pause-on-hover" class="scroll-mt-20 flex flex-col gap-3">
			<h3
			...
			>
				<div
					class="overflow-hidden rounded-[var(--radius-lg)] border border-border bg-card shadow-[var(--shadow-sm)]"
				>
					<div class="relative flex min-h-[6rem] items-center bg-secondary/40 px-6">
						<Marquee pauseOnHover class="py-4">
							{#each logos as logo (logo)}
								<span class="text-[0.86rem] font-[var(--font-weight-label,500)] text-foreground"
									>{logo}</span
								>
							{/each}
						</Marquee>
		...
		<!-- Vertical marquee -->
		<div id="vertical-marquee" class="scroll-mt-20 flex flex-col gap-3">
			<h3
				class="text-[1rem] font-[var(--font-weight-header,600)] tracking-tight text-foreground docs-subsection-heading"
			>
				Vertical marquee
			...
			<ComponentPreview
				code={`<Marquee vertical pauseOnHover>\n  {#each logos as logo (logo)}\n    <span>{logo}</span>\n  {/each}\n</Marquee>`}
			>
				<div
					class="overflow-hidden rounded-[var(--radius-lg)] border border-border bg-card shadow-[var(--shadow-sm)]"
				>
					<div
						class="relative flex min-h-[14rem] w-full items-center justify-center bg-secondary/40"
					>
						<Marquee vertical pauseOnHover class="py-4">
							{#each logos as logo (logo)}
								<span class="text-[0.86rem] font-[var(--font-weight-label,500)] text-foreground"
									>{logo}</span
								>
		...
		<!-- Custom duration -->
		<div id="custom-duration" class="scroll-mt-20 flex flex-col gap-3">
			<h3
				class="text-[1rem] font-[var(--font-weight-header,600)] tracking-tight text-foreground docs-subsection-heading"
			>
				Custom duration
			...
			<ComponentPreview
				code={`<Marquee duration="20s">\n  {#each logos as logo (logo)}\n    <span>{logo}</span>\n  {/each}\n</Marquee>`}
			>
				<div
					class="overflow-hidden rounded-[var(--radius-lg)] border border-border bg-card shadow-[var(--shadow-sm)]"
				>
					<div class="relative flex min-h-[6rem] items-center bg-secondary/40 px-6">
						<Marquee duration="20s" class="py-4">
							{#each logos as logo (logo)}
								<span class="text-[0.86rem] font-[var(--font-weight-label,500)] text-foreground"
									>{logo}</span
								>
	...
	<section id="api" class="scroll-mt-20 flex flex-col gap-4">
		...
		<p class="text-sm text-foreground-muted">
			<code class="font-mono text-foreground">Marquee</code> accepts children and animation props. All
			CSS props are customizable via your theme.
		</p>
