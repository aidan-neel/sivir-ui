<script lang="ts">
	import { flyAndScale, flyNoOpacity } from '$lib/silk/internals/transition';
...
{#if uiState.data.open}
	...
	<div
		...
		class={cn(
			className,
			`bg-[var(--color-panel)] border-l border-[var(--panel-border)] shadow-[var(--panel-shadow)] p-[var(--panel-padding-lg)] transition-all fixed flex flex-col top-0 right-0 z-50 overflow-y-auto m-auto w-full max-w-[25rem] h-screen`
		)}
		use:clickOutside={() => {
