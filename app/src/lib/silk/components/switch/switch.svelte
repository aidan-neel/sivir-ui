...
<div class="flex flex-row items-start gap-2.5">
	...
	{#if label || description}
		<div class="flex min-w-0 flex-col gap-0.5 pt-px">
			{#if label}
				<span id={labelId} class="text-[length:var(--text-sm)] font-medium text-foreground [font-family:var(--font-sans),sans-serif] select-none">
					{label}
				</span>
			{/if}
			{#if description}
