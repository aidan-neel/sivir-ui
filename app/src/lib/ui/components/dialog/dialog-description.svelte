<script lang="ts">
	...
	import type { DialogDescriptionProps, DialogState } from ".";
...
<p {...rest} id={uiState.key + '-desc'} class={cn(className, `text-sm text-center sm:text-left text-foreground-muted py-1`)}>
    {@render children?.()}
</p>