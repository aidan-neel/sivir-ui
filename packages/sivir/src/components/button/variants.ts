...
export const button = tv({
	base: 'inline-flex h-[var(--button-height)] hover:cursor-[var(--ui-cursor-interactive)] items-center justify-center gap-[var(--button-gap)] whitespace-nowrap select-none rounded-[var(--radius-lg)] px-[var(--button-padding-x)] [font-size:var(--font-size-button)] [font-weight:var(--font-weight-button)] [letter-spacing:var(--tracking-button)] leading-none antialiased transition-[background-color,border-color,color,box-shadow,transform] [transition-duration:var(--motion-duration-press)] ease-[var(--ease-out)] active:scale-[var(--motion-press-scale)] focus-visible:outline-none focus-visible:ring-0 focus-visible:shadow-[var(--focus-ring)] disabled:pointer-events-none disabled:opacity-40 [&_svg]:pointer-events-none [&_svg:not([class*="size-"])]:size-4 [&_svg]:shrink-0',
	variants: {
		variant: {
			...
			panel:
				'border border-[var(--panel-border,var(--color-border))] bg-[var(--panel-bg,var(--color-card))] text-[var(--panel-fg,var(--color-foreground))] shadow-[var(--card-shadow)] ring-1 ring-inset ring-[color-mix(in_oklab,var(--panel-border,var(--color-border))_50%,transparent)] hover:bg-[var(--button-outlined-hover-bg)] data-[state=open]:bg-[var(--button-outlined-hover-bg)] focus-visible:shadow-[var(--focus-ring),var(--card-shadow)]' // token-lint-disable-line no-literal-length
		},
		size: {
