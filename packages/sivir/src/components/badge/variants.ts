...
export const badge = tv({
	base: 'flex items-center justify-center rounded-[var(--radius-lg)] px-[var(--sivir-space-2)] py-0.5 [font-size:var(--font-size-badge,12px)] leading-[1.2] [font-weight:var(--font-weight-badge,500)] [letter-spacing:var(--tracking-badge,0em)] transition-[background-color,border-color,color] [transition-duration:var(--motion-duration-hover)] ease-in-out motion-reduce:transition-none hover:cursor-default disabled:cursor-not-allowed disabled:opacity-50', // token-lint-disable-line no-literal-length,no-primitive-leak
