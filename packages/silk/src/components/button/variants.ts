...
export const button = tv({
	base: 'flex h-[var(--button-height)] hover:cursor-[var(--ui-cursor-interactive)] select-none items-center justify-center gap-[var(--button-gap)] whitespace-nowrap rounded-[var(--radius-lg)] px-[var(--button-padding-x)] [font-size:var(--font-size-button,14px)] [font-weight:var(--font-weight-button,500)] [letter-spacing:var(--tracking-button,0em)] leading-none antialiased transition-[background-color,border-color,color,opacity,box-shadow,transform] duration-[var(--motion-duration-hover)] ease-[var(--motion-easing-hover,cubic-bezier(0.25,0.1,0.25,1))] [transform:translateZ(0)] active:[transform:translate3d(0,var(--haptic-press-y,0px),0)] [backface-visibility:hidden] focus-visible:outline-none focus-visible:ring-0 focus-visible:shadow-[var(--ui-button-shadow,0_0_#0000),0_0_0_3px_var(--color-ring)] disabled:pointer-events-none disabled:opacity-40 [&_svg]:pointer-events-none [&_svg:not([class*="size-"])]:size-4 [&_svg]:shrink-0',
	variants: {
		variant: {
			primary:
